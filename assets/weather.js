                ...
                function getDailyForecast(city) {
                    ...
                    $.ajax({
                        ...
                        for (var i = 0, j = 1; i < response.list.length, j <= 5; i += 8, j++) {
                            // Convert our date to human readable format.
                            var dateTime = convertDate(response.list[i].dt);
                            // Create, style, and set to DOM.
                            ...
                            pTemp.html('<i></i>' + response.list[i].main.temp).appendTo(newDiv);
                            pHumid.html('<i></i>' + response.list[i].main.humidity + "%").appendTo(newDiv);
